...
export default function App() {
  ...
  async function handleLikeRepository(id) {
    ...
    let reposUpdated = repos.map(item => {
      if (item.id === id) {
        item.likes = totalLikes
      }
    ...
    return (
      <View style={styles.repositoryContainer}>
        ...
        <View style={styles.techsContainer}>
          {props.techs.map(tech => <Text key={Math.random()} style={styles.tech}>{tech}</Text>)}
        </View>
        ...
        <TouchableOpacity
          style={styles.button}
          onPress={() => handleLikeRepository(props.id)}
